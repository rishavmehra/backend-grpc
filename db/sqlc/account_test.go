...
func TestGetAccount(t *testing.T) {
	// Create a random account in the database
	account1 := createRandomAccount(t)

	// Retrieve the account using GetAccount function
	account2, err := testQuries.GetAccount(context.Background(), account1.ID)

	// Assert no error occurred during retrieval
	require.NoError(t, err)
	// Assert the retrieved account is not empty
	require.NotEmpty(t, account2)

	// Assert the retrieved account's fields match the original account's fields
	require.Equal(t, account1.ID, account2.ID)
	require.Equal(t, account1.Owner, account2.Owner)
	require.Equal(t, account1.Balance, account2.Balance)
	require.Equal(t, account1.Currency, account2.Currency)
	// Assert the CreatedAt timestamps are within one second of each other
